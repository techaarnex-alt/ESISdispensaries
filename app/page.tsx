...
export default function Home() {
  ...
  useEffect(() => {
    ...
    register({ name: 'create_location_test_entry', title: 'Create location test entry', description: 'Create a new laboratory test entry in the signed-in dispensary register. Use only after the patient details and requested tests are known.', inputSchema: { type: 'object', properties: { patient: { type: 'string' }, ipNumber: { type: 'string' }, age: { type: 'string' }, sex: { type: 'string', enum: ['Male', 'Female', 'Other'] }, relationship: { type: 'string' }, ipHolderName: { type: 'string' }, tests: { type: 'array', items: { type: 'string' }, minItems: 1 } }, required: ['patient', 'ipNumber', 'tests'], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute: async (input) => {
      const data = input as { patient: string; ipNumber: string; age?: string; sex?: string; relationship?: string; ipHolderName?: string; tests: string[] };
      const branch = locationRef.current;
      if (!branch) throw new Error('A dispensary login is required.');
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch('/api/entries', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ patient, ipNumber: ip, age: String(form.get('age') || ''), sex: String(form.get('sex') || ''), relationship: String(form.get('relationship') || 'Self'), ipHolderName: String(form.get('ipHolderName') || ''), tests: tests.map((test) => ({ testId: test.id, ratePaise: test.ratePaise })) }) });
    ...
    formElement.reset(); setEntries((current) => [data.entry!, ...current]); setPreview(data.entry); setTests([]); setEntryFormVersion((current) => current + 1); setSection('reports'); notify('Test entry was stored for this location only.');
  };
  const signOut = async () => {
