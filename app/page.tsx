...
import { AlertTriangle, ArrowUpRight, Bell, CalendarDays, CheckCircle2, ChevronRight, ClipboardList, FileText, FlaskConical, LayoutDashboard, LogOut, MapPin, Menu, MessageSquare, Pencil, Plus, Printer, Save, Search, ShieldCheck, Star, Users, X } from 'lucide-react';
...
function Preview({ entry, location, close }: { entry: TestEntry; location: Branch; close: () => void }) {
  ...
  const reportRows = results.length ? results.map((result) => ({ name: result.testName, value: result.resultValue || '—', referenceRange: result.referenceRange || '—' })) : entry.tests.map((test) => ({ name: test, value: '—', referenceRange: '—' }));
