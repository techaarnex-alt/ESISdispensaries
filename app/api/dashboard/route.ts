...
export async function GET(request: Request) {
  ...
  const entries = (entryResult.results as unknown as EntryRow[]).map((entry) => ({ recordId: entry.id, id: entry.report_number, patient: entry.patient, registrationNumber: entry.registration_number, ip: entry.ip_number, age: entry.age, sex: entry.sex, relationship: entry.relationship, ipHolderName: entry.ip_holder_name, transferredFrom: entry.transferred_from_location_id ? branchById(entry.transferred_from_location_id)?.name || '' : '', tests: JSON.parse(entry.tests_json) as string[], date: new Intl.DateTimeFormat('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(entry.created_at)) }));
