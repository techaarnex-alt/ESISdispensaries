...
export async function POST(request: Request) {
  ...
  const transferredFrom = transferredFromLocationId && transferredFromLocationId !== branch.id ? branchById(transferredFromLocationId) : undefined;
  ...
  const available = await db().prepare('SELECT id, name FROM lab_tests WHERE location_id = ? AND active = 1 ORDER BY name').bind(branch.id).all<TestRow>();
  ...
  const created = await db().prepare('INSERT INTO lab_entries (report_number, location_id, patient, registration_number, ip_number, age, sex, relationship, ip_holder_name, transferred_from_location_id, tests_json, status, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').bind(reportNumber, branch.id, patient, registrationNumber, ipNumber, age, sex, relationship, ipHolderName, transferredFrom?.id || null, JSON.stringify(distinct.map((test) => test.name)), 'Collected', now).run();
  const recordId = Number(created.meta.last_row_id);
  if (!recordId) return json({ error: 'Unable to allocate this test entry.' }, 500);
  await db().batch(distinct.map((test) => db().prepare('INSERT INTO lab_entry_tests (entry_id, test_id, test_name, result_value, result_note, result_status, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)').bind(recordId, test.id, test.name, '', '', 'Pending', now)));
  return json({ entry: { recordId, id: reportNumber, patient, registrationNumber, ip: ipNumber, age, sex, relationship, ipHolderName, transferredFrom: transferredFrom?.name || '', tests: distinct.map((test) => test.name), status: 'Collected', date: new Intl.DateTimeFormat('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).format(new Date(now)) } }, 201);
}
